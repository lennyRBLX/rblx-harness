...
- Use a disposable place; human selects Studio/live & reports each result (TEST1). Stage disabled fixtures with `tools/create_boilerplate/create_boilerplate.py --test`; author source in stopped Edit. Assign exact paths & cleanup ownership; preserve other edits.
...
Recheck these project procedures against current operational sources & versions.
